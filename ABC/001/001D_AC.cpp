...
int main()
{
    ...
    rep(i,0,N) {
        ...
        ll T1 = stoll(S), T2 = stoll(E);
        T1 = (T1/100)*60 + T1%100;
        T2 = (T2/100)*60 + T2%100;
        T1 = floor((long double)T1/5)*5;
        T2 = ceil((long double)T2/5)*5;
        v.emplace_back(T1,T2);
    }
    sort(v.begin(),v.end());

    ll dwn = v[0].first, upn = v[0].second;
    rep(i,1,v.size()) {
        if (v[i].first <= upn) 
            upn = max(upn,v[i].second);
        else {
            ret.emplace_back(dwn,upn);
            dwn = v[i].first;
            upn = v[i].second;
        }
    }
    ...
    rep(i,0,ret.size()) {
        cout << leadingzeros(ret[i].first/60,2) << leadingzeros(ret[i].first%60,2);
        cout << "-";
        cout << leadingzeros(ret[i].second/60,2) << leadingzeros(ret[i].second%60,2);
        cout << endl;
    }
