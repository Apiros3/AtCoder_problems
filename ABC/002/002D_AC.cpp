...
int main()
{
    ...
    ll arr[20][20];
    rep(i,0,20) {rep(j,0,20) arr[i][j]=0;}
    rep(i,0,20) arr[i][i]=1;
    rep(i,0,M) {
        arr[X[i]][Y[i]]=1;
        arr[Y[i]][X[i]]=1;
    }
    ...
    rep(i,1,1<<N) {
        ll counter=0;
        string A = basechange_str(10,2,to_string(i));
        A = leadingzeros(stoll(A),N);
        bool tmp = 1;
        vector<ll> v(0);
        rep(j,0,A.length()) if (A[j]=='1') v.push_back(j+1);
        rep(j,0,v.size()) {
            rep(k,0,v.size()) {
                if (arr[v[j]][v[k]]==0) tmp=0;
            }
        }
        if (tmp) 
            ans = max(ans, v.size());
